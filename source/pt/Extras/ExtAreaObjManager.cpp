...
void ExtAreaObjManagers(AreaObjMgr* pManager, AreaObjContainer* pContainer, const JMapInfoIter& rIter, s32 l) {
    ...
    if (l == 94) {
        for (s32 i = 0; i < cNumNewMgrs; i++) {
            ManagerEntry* entry = &cNewMgrTable[i];
...
kmWrite32(0x80071318, 0x3860017C + cNumNewMgrs*4); // li r3, 0x17C + cNumNewMgrs*4
kmWrite32(0x80071320, 0x3800005F + cNumNewMgrs); // li r0, 0x5F + cNumNewMgrs
